...
const GabruOnline: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* GitHub Button (Triggers the Popover) */}
      <button
        className={`${styles.githubBtn} ${isOpen ? styles.githubBtnActive : ''}`}
        onClick={() => setIsOpen((prev) => !prev)}
        aria-expanded={isOpen}
        aria-label="Toggle creator profiles"
      >
        <svg
          className={styles.triggerGithubIcon}
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
      ...
      {/* ── Status Badge (Non-clickable, shows count) ── */}
